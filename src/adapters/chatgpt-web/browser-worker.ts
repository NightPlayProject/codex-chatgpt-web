...
export class ChatGptBrowserWorker {
  ...
  private async selectModelAndEffort(
    ...
    // Some accounts expose enabled Pro as a separate picker row instead of a fifth
    // slider tick. Keep that verified path while reading the slider atomically.
    if (uiEffortIndex === 4) {
      const snapshot = await readChatGptEffortSnapshot(sliderContainer)
        .catch(error => { throw chatGptModelControlUnavailableAdapterError(String(error)); });
      if (uiEffortIndex > snapshot.max - snapshot.min) {
  ...
  private async submissionDomState(
    ...
    const observed = await withChatGptBrowserObservationTimeout(withBrowserTurnAbort(page.evaluate(options => {
      ...
      groups.forEach((group, index) => {
        ...
        if (group.querySelector('[data-conversation-role="assistant"], [data-chatgpt-agent-turn-start], [class~="group/activity-header"]')) {
  ...
  private async waitForNewAssistantTurn(
    ...
    for (;;) {
      ...
      const pairedIdentity = chatGptSubmittedAssistantIdentity(
        observationBaseline.initialTurnIdentities,
        state.userIdentities,
        state.responseIdentities,
      );
      // Activity can replace the submitted user group with a temporary assistant
      // group before the user bubble returns. Once Send is accepted, bind the one
      // newly mounted assistant group when no new user group is visible. The
      // later rebind still has to prove the returned user and completed answer.
      const identity = pairedIdentity ?? (
        (observationBaseline.acceptedUserIdentity || observationBaseline.submittedText)
          && !chatGptNewTurnIdentity(observationBaseline.initialTurnIdentities, state.userIdentities)
          ? chatGptNewTurnIdentity(
            observationBaseline.initialTurnIdentities,
            state.responseIdentities.filter(turn => turn.startsWith("group:assistant:")),
          )
          : undefined
      );
      if (progress
  ...
  private async reconcileAssistantTurnBinding(
    ...
    if (boundCount === 1) {
      const bound = await this.responseDomSnapshot(binding.locator, {});
      // The stable turn key may outlive its Activity or answer contents. An
      // empty shell does not prove that the submitted response is still here.
      if (bound.visibleText.trim() || bound.traceBlocks.some(block => block.text.trim())
        || bound.completionActionVisible) return binding;
    }
    if (boundCount > 1) {
  ...
  private async responseDomSnapshot(
    ...
    const observed = await responseTurn.evaluate((element, options) => {
      ...
      function isAgentActivityCommentary(candidate: HTMLElement) {
        if (candidate.closest('[data-user-message-bubble]')) return false;
        if (candidate.closest('[data-content-search-unit-key]')) return false;
      ...
      const selectChatGptAnswerRoots = (
        ...
        const agentActivityRoots = markdownRoots.filter(candidate => (
          isAgentActivityCommentary(candidate)
          || (!candidate.closest('[data-content-search-unit-key]')
            && activityContainers.some(container => container.contains(candidate)))
        ));
