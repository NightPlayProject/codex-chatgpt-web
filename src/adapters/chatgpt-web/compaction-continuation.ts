...
export class ChatGptCompactionContinuationStore {
  ...
  /**
   * Same-process proof that the exact source revision already produced a successful structured
   * handoff, but the enclosing Responses callback has not necessarily persisted the checkpoint
   * yet. This closes the ordering gap where retained-source cleanup supersedes the old observer
   * before server.ts receives the completed compact response.
   *
   * These entries are deliberately memory-only. The normal completed-response path remains the
   * only cross-restart authority.
   */
  private readonly completedHandoffSources = new Map<string, ReadonlySet<string>>();
  ...
  remember(
    ...
    this.completedHandoffSources.delete(key);
  ...
  /**
   * Return whether this exact native source revision already produced a completed checkpoint.
   *
   * Unlike `accepts`, this intentionally does not require the caller to replay the summary. It is
   * used only to classify the *old* response that Codex is retiring after the compact request has
   * already completed and persisted its checkpoint.
   */
  hasCompletedSource(
    parsed: CodexParsedRequest,
    identity: ChatGptTurnIdentity,
    ...
    return this.checkpoints.get(key)?.sourceHashes.has(hash) === true
      || this.completedHandoffSources.get(key)?.has(hash) === true;
  }

  /** Record a successful structured handoff before retained-source cleanup races its observer. */
  rememberCompletedHandoffSource(
    parsed: CodexParsedRequest,
    identity: ChatGptTurnIdentity,
    source: ChatGptTurnUserRevision,
  ): void {
    ...
    this.completedHandoffSources.delete(key);
    this.completedHandoffSources.set(key, new Set([sourceDigest(source)]));
    while (this.completedHandoffSources.size > MAX_CHECKPOINTS) {
      const oldest = this.completedHandoffSources.keys().next().value as string | undefined;
      if (!oldest) break;
      this.completedHandoffSources.delete(oldest);
    }
  }
...
/** Completed-compaction evidence for retiring the exact pre-compaction source response. */
export function hasCompletedCompactionForSource(
  parsed: CodexParsedRequest,
  identity: ChatGptTurnIdentity,
  source: ChatGptTurnUserRevision,
): boolean {
  return storeFor(parsed).hasCompletedSource(parsed, identity, source);
}

/**
 * Same-process bridge between successful ChatGPT handoff and the server's completed-response
 * persistence callback. This never creates cross-restart continuation authority.
 */
export function rememberCompletedCompactionHandoffSource(
  parsed: CodexParsedRequest,
  identity: ChatGptTurnIdentity,
  source: ChatGptTurnUserRevision,
): void {
  storeFor(parsed).rememberCompletedHandoffSource(parsed, identity, source);
}
