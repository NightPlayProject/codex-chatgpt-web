...
export function compileChatGptWebPrompt(
  ...
  const transportContract = parsed._compactionRequest
    ...
    ? [
      ...
      "If codex_tool_capabilities reports local_execution_recovery.available=true, use codex_exec for commands and filesystem work, and codex_write_stdin for a returned session_id. The current native command handler is absent, so do not invoke tool names remembered from an earlier round.",
