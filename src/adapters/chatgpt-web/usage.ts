...
export function estimateChatGptWebUsage(
  ...
  const nativeHistoryTokens = estimateNativeSolHistoryTokens(parsed);
  const toolCatalogTokens = estimateNativeSolToolCatalogTokens(parsed);
  const inputTokens = Math.max(
    Math.max(browserInputTokens, nativeHistoryTokens) + toolCatalogTokens,
    parsed._nativeUsageInputTokenFloor ?? 0,
  );
...
/**
 * Estimate the canonical native Codex input independently of the smaller browser reconstruction.
 * Server-side transition compaction uses this before opening ChatGPT so an inherited native task
 * that already crossed the Web route threshold can be compacted once instead of failing preflight.
 */
export function estimateChatGptWebNativeInputTokens(parsed: CodexParsedRequest): number {
  return estimateNativeSolHistoryTokens(parsed) + estimateNativeSolToolCatalogTokens(parsed);
}

function estimateNativeSolHistoryTokens(parsed: CodexParsedRequest): number {
  return Math.max(
