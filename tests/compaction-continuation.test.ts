...
test("completed source evidence can retire the pre-compaction response without replaying the summary", () => {
  const root = mkdtempSync(join(tmpdir(), "codex-completed-source-"));
  temporaryRoots.push(root);
  const path = join(root, "compaction-continuations.json");
  const compact = checkpointFixture({ compaction: true });
  const identity = extractChatGptTurnIdentity(compact.parsed);
  const store = new ChatGptCompactionContinuationStore(path);
  store.remember(compact.parsed, identity, [compact.source], compact.summary);

  // The retiring source request predates the compact output, so it cannot replay the summary.
  delete compact.parsed._compactionRequest;
  expect(new ChatGptCompactionContinuationStore(path).hasCompletedSource(
    compact.parsed,
    identity,
    compact.source,
  )).toBeTrue();

  const changed = checkpointFixture({
    compaction: true,
    sourceText: "A genuinely newer human instruction",
  });
  expect(new ChatGptCompactionContinuationStore(path).hasCompletedSource(
    compact.parsed,
    identity,
...
test("successful handoff source evidence closes the pre-persistence race without becoming restart authority", () => {
  const root = mkdtempSync(join(tmpdir(), "codex-completed-handoff-source-"));
  temporaryRoots.push(root);
  const path = join(root, "compaction-continuations.json");
  const compact = checkpointFixture({ compaction: true });
  const identity = extractChatGptTurnIdentity(compact.parsed);
  ...
  store.rememberCompletedHandoffSource(compact.parsed, identity, compact.source);
  delete compact.parsed._compactionRequest;
  expect(store.hasCompletedSource(compact.parsed, identity, compact.source)).toBeTrue();

  const mismatched = checkpointFixture({
    compaction: true,
    sourceText: "A different current instruction must not inherit the handoff receipt",
  });
  expect(store.hasCompletedSource(
    mismatched.parsed,
    extractChatGptTurnIdentity(mismatched.parsed),
    mismatched.source,
  )).toBeFalse();

  // A later same-scope handoff replaces the transient receipt instead of authorizing both
  // concurrent source revisions.
  store.rememberCompletedHandoffSource(
    mismatched.parsed,
    extractChatGptTurnIdentity(mismatched.parsed),
    mismatched.source,
  );
  expect(store.hasCompletedSource(mismatched.parsed, identity, compact.source)).toBeFalse();
  expect(store.hasCompletedSource(mismatched.parsed, identity, mismatched.source)).toBeTrue();

  const otherTurn = checkpointFixture({ compaction: true, turnId: "turn-other" });
  expect(store.hasCompletedSource(
    otherTurn.parsed,
    extractChatGptTurnIdentity(otherTurn.parsed),
    mismatched.source,
  )).toBeFalse();

  // Same-process handoff evidence must not survive a restart until the completed Responses
  // callback persists the canonical checkpoint.
  expect(new ChatGptCompactionContinuationStore(path).hasCompletedSource(
    compact.parsed,
    identity,
    compact.source,
